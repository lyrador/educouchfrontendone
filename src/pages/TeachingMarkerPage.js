...
import { Grid, Typography, Button, TextField, Dialog, DialogActions, DialogContent, Alert, Snackbar,
     DialogContentText, DialogTitle, Card, CardContent, Box, Breadcrumbs, Paper } from '@mui/material';
...
function TeachingMarkerPage() {
    ...
    const handleOnChange = (val,index) => {
        ...
        setQuestions(newArr)
        const newTotal = newArr.reduce((total, curr) => total + curr.questionAttemptScore, 0);
    ...
    const handleOnCompleteGrading = async () => {
        try {
            const response = await axios.post(
              "http://localhost:8080/gradeBookEntry/updateOpenEndedQuestions?learnerId="+ learnerId +"&assessmentId="+ assessmentId, questions
            );
            // set the state of the user
            // store the user in localStorage
            console.log(response)
            refreshPage()
            navigate(gradingPath, {state:{name: name ,title: title }})
    ...
    React.useEffect(() => {
        fetch("http://localhost:8080/gradeBookEntry/getOpenEndedQuestions?learnerId="+learnerId+"&assessmentId="+assessmentId)
            .then(res => res.json())
            .then((result) => {
                setQuestions(result);
                console.log(result)
                ...
                for(let i = 0; i<result.length;i++) {
                    ...
                    totalOE += parseInt(result[i].questionAttempted.questionMaxPoints)
                    totalLearnerOE += parseInt(result[i].questionAttemptScore)
                    // setTotalOpenEndedScore((totalOpenEndedScore + parseInt(result[i].questionAttempted.questionMaxPoints)))
                    // setTotalLearnerOpenEnded(totalLearnerOpenEnded +parseInt(result[i].questionAttemptScore))
                }
    ...
    return (
        <div>
            ...
            return (
              <Paper elevation={3} style={{ padding: 30, marginTop: 20 }} key={question.questionAttemptId}>
