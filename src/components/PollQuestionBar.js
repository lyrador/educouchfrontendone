...
export default function PollQuestionBar(props) {
    ...
    //create
    const [newPollQuestionTitle, setNewPollQuestionTitle] = useState("");
    ...
    //delete
    const [pollQuestionIdToDelete, setPollQuestionIdToDelete] = useState("");
    const [deleteDialogOpen, setDeleteDialogOpen] = React.useState(false);
    const [deleteAllDialogOpen, setDeleteAllDialogOpen] = React.useState(false);

    const handleClickDeleteDialogOpen = (event, pollQuestionId) => {
        setPollQuestionIdToDelete(pollQuestionId);
    ...
    const handleQuestionChange = (event, pollQuestionId, pollQuestionNumber) => {
        // props.setCheckForChanges(true)
        // if (props.changesToQuestionWasMade || props.questionIdToBrowse == "") {
        props.setQuestionIdToBrowse(pollQuestionId)
        props.setQuestionNumberToBrowse(pollQuestionNumber)
        // }
    ...
    const createNewQuestion = async (e) => {
        ...
        if (newPollQuestionTitle) {
            ...
            try {
                const response = await fetch("http://localhost:8080/pollQuestion/" + pollId + "/pollQuestions", {
                    method: "POST",
                ...
                if (response.ok == false) {
                    ...
                    console.log("New Poll Question Created Successfully!");
                    handleClickSnackbar()
                }
            } catch (err) {
                console.log(err);
    ...
    const deleteQuestion = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch("http://localhost:8080/pollQuestion/pollQuestions/" + pollQuestionIdToDelete, {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
            })
            console.log(response);
            if (response.ok == false) {
                ...
                console.log("Poll Question Deleted Successfully!");
    ...
    const deleteAllChapters = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch("http://localhost:8080/interactiveChapter/interactiveChapters/" + pollId + "/deleteAll", {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
            })
            console.log(response);
    ...
    const editChapter = async (e) => {
        ...
        if (newPollQuestionTitle && newQuestionTimeLimit) {
            var interactiveChapterTitle = newPollQuestionTitle
    ...
    const reorderQuestions = async (e) => {
        ...
        for (var i = 0; i < props.questionsToBeReordered.length; i++) {
            props.questionsToBeReordered[i].pollQuestionNumber = i + 1
        }
        try {
            const response = await fetch("http://localhost:8080/pollQuestion/poll/" + pollId + "/reorderQuestions", {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                            ...
                            <div>
                                <div style={{ width: "100%", display: "flex" }}>
                                    <div style={{ width: "10%", paddingLeft: "10px", paddingTop: '6px' }}>
                                        <IconButton
                                            aria-label="settings"
                                            onClick={(event) => handleClickDeleteDialogOpen(event, question.pollQuestionId)}
                                        >
                                    ...
                                    <div className="deleteJiggle" style={{ width: "85%", paddingLeft: "8px" }} >
                                        <Button fullWidth style={{ justifyContent: "flex-start" }}>
                                            <div className="chapterLine">Q{question.pollQuestionNumber} - {question.pollQuestionTitle}</div>
                                        </Button>
                                    </div>
                                </div>
                                <Divider />
                ...
                <Dialog open={open} onClose={handleClose} maxWidth={'sm'} fullWidth={true}>
                    ...
                    <DialogContent>
                        <TextField
                            ...
                            required
                            style={{ margin: "6px 0" }}
                            value={newPollQuestionTitle}
                            onChange={(e) => setNewPollQuestionTitle(e.target.value)}
                    ...
                    <DialogContent>
                        <div style={{ width: '60%', margin: 'auto', display: 'flex' }}>
                            <div style={{ width: '15%' }}>
                                ...
                                <ul style={{ listStyleType: 'none' }}>
                                    {props.questionsToBeReordered.map(({ pollQuestionNumber, chapterTitle }, index) => {
                                        return (
                                            <li>
                                                <div style={{ display: 'flex', width: '100%' }}>
                                    ...
                                    <Droppable droppableId="chaptersReordering">
                                        {(provided) =>
                                            <ul {...provided.droppableProps} ref={provided.innerRef} style={{ listStyleType: 'none' }}>
                                                {props.questionsToBeReordered.map(({ pollQuestionNumber, pollQuestionTitle }, index) => {
                                                    return (
                                                        <Draggable key={pollQuestionNumber} draggableId={pollQuestionNumber} index={index}>
                                                            {(provided) => (
                                                                <li {...provided.draggableProps} {...provided.dragHandleProps} ref={provided.innerRef}>
                                                                    <div style={{ display: 'flex', width: '100%' }}>
                                                                        {((index + 1).toString() == pollQuestionNumber) &&
                                                                            <div style={{ borderStyle: 'solid', borderWidth: '1px', marginBottom: '2px', width: '100%', borderRadius: '10px' }}>
                                                                                <h3 style={{ marginLeft: '5px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{pollQuestionNumber} - {pollQuestionTitle}</h3>
                                                                            </div>
                                                                        }
                                                                        {((index + 1).toString() != pollQuestionNumber) &&
